...
export const LevelPlayer: React.FC<LevelPlayerProps> = ({ level, onClose, onComplete }) => {
  ...
  if (level.type === LevelType.FLASHCARDS) {
    ...
    return (
      <div className="fixed inset-0 z-[60] bg-gray-900 flex flex-col items-center justify-center p-6 overflow-hidden">
         {/* Top Bar */}
         <div className="absolute top-0 left-0 right-0 p-6 flex justify-between items-center text-white/60 font-bold uppercase tracking-widest text-sm z-10">
            <button onClick={onClose} className="hover:text-white transition-colors"><X /></button>
            <div className="flex flex-col items-center">
               <span>Lernstapel</span>
               <div className="flex gap-1 mt-1">
                  <span className="text-green-400">{completedCards} Fertig</span>
                  <span className="text-white/20">|</span>
                  <span className="text-orange-400">{cardQueue.length} Offen</span>
               </div>
            </div>
            <div className="w-6" />
         </div>
         ...
         {/* Card Container */}
         <div className="w-full max-w-sm aspect-[3/4] relative perspective-1000 cursor-pointer group mb-8" onClick={() => setIsFlipped(!isFlipped)}>
            <div className={`relative w-full h-full transition-transform duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
               
               {/* Front */}
               <div className={`absolute inset-0 backface-hidden bg-white rounded-[2rem] shadow-2xl flex flex-col items-center justify-center p-8 border-b-[12px] border-gray-200`}>
                  <div className="absolute top-8 text-gray-300 font-black text-8xl opacity-20 select-none">?</div>
               ...
               {/* Back */}
               <div className={`absolute inset-0 backface-hidden rotate-y-180 ${currentColor} rounded-[2rem] shadow-2xl flex flex-col items-center justify-center p-8 border-b-[12px] border-black/20 text-white`}>
